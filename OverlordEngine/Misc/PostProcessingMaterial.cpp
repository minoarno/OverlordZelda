...
void PostProcessingMaterial::DrawPass(const SceneContext& sceneContext, ID3DX11EffectTechnique* pTechnique, RenderTarget* pDestination)
{
	...
	pDestination->Clear(DirectX::XMFLOAT4{ Colors::Purple });
	...
	//Set PrimitiveTopology (TRIANGLELIST)
	sceneContext.d3dContext.pDeviceContext->IASetPrimitiveTopology(D3D10_PRIMITIVE_TOPOLOGY_TRIANGLESTRIP);

	//Set VertexBuffer

	UINT val = sizeof(VertexPosTex);
	UINT offset{};
	sceneContext.d3dContext.pDeviceContext->IASetVertexBuffers(0, 1, &m_pDefaultVertexBuffer, &val, &offset);
